...
<script type="text/javascript">
	...
	function doEdit(){
		...
		if (rows.length===1){
			...
			$("#staff .combo-text").val(rows[0].staff.name);
			$("#staff .combo-value").val(rows[0].staff.id);
			$("#editSubareaGrid").datagrid({
				fitColumns:true,
				singleSelect:false,
				border:false,
				url:'${pageContext.request.contextPath}/decidedZoneAction_associationSubarea?id='+rows[0].id,
				columns:[[{
					field : 'subareaid',
					width:30,
					checkbox:true
			   ...
			   onLoadSuccess:function(data){
					for (var i=0;i<data.rows.length;i++){
						$("#editSubareaGrid").datagrid('selectRow',i);
					}
			}
			});
			$("#editDecidedzoneForm").form("load",rows[0]);

		} else {
			$.messager.alert("选择定区","请选择一个定区","warning");
		}
	}
	...
	function doDelete(){
		...
		if (arr.length==0){
			$.messager.alert("选择删除的定区","请选择需要删除的定区！","warning");
		}else{
			window.location.href="${pageContext.request.contextPath}/decidedZoneAction_delete?ids="+arr.join(',');
		}
	}
	function doSearch(){
	...
	function doAssociations(){
		...
		if (arr.length===0){
			...
			$.post(
					"${pageContext.request.contextPath}/decidedZoneAction_noassociation",
					function (data) {
						if (data.length!==0){
			...
			$.post(
					"${pageContext.request.contextPath}/decidedZoneAction_association",
					{"id":id},
					function (data) {
						if (data.length!==0){
							for (var i=0;i<data.length;i++){
							...
							var rightSelect=document.getElementById("associationSelect");
							var leftSelect=document.getElementById("noassociationSelect");
	...
	$(function(){
		...
		$('#grid').datagrid( {
			...
			pagination : true,
			toolbar : toolbar,
			url : "${pageContext.request.contextPath}/decidedZoneAction_list",
			idField : 'id',
			columns : columns,
		...
		$("#edit").click(function () {
			...
			if (flag){
				$("#editDecidedzoneWindow").window("close");
	...
	function doDblClickRow(index,row){
		$('#association_subarea').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "${pageContext.request.contextPath}/decidedZoneAction_associationSubarea?id="+row.id,
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "${pageContext.request.contextPath}/decidedZoneAction_association?id="+row.id,
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="staff.id"
    							data-options="valueField:'id',textField:'name',mode:'remote',url:'${pageContext.request.contextPath}/staffAction_staffList'" />
						</td>
					</tr>
					<tr height="300">
						...
						<td>
							<table id="subareaGrid"  class="easyui-datagrid" border="false" style="width:300px;height:300px"
								   data-options="url:'${pageContext.request.contextPath}/subareaAction_subareaList',
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="editDecidedzoneForm" action="${pageContext.request.contextPath}/decidedZoneAction_edit" method="post">
				<table class="table-edit" width="80%" align="center">
					...
					<tr height="300" class="associationSub">
						<td valign="top">关联分区</td>
						<td>
							<table id="editSubareaGrid"  style="width:300px;height:300px" class="editSubareaGrid">
							</table>
