...
<script type="text/javascript">
	...
	$(function() {
		...
		// 基本功能菜单加载
		$.ajax({
			url : '${pageContext.request.contextPath}/userAction_findMenu',
			type : 'POST',
			dataType : 'text',
	...
	function checkNewPassword() {
		if ($('#txtNewPass').val()!==$('#txtRePass').val()) {
			$('#errorForNewPass').html("两次密码不一致");
			setTimeout(function () {
				$('#errorForNewPass').html("");
			},500)
		}
	}
	function updatePassword() {
		...
		if (flag){
			$.post(
					'${pageContext.request.contextPath}/userAction_editPass',
...
<body class="easyui-layout">
	<div data-options="region:'north',border:false"
		...
		<div id="layout_north_kzmbMenu" style="width: 100px; display: none;">
			...
			<div onclick="logoutFunc();">退出系统</div>
		</div>
	</div>
...
<%--			</div>--%>
	...
	<!--修改密码窗口-->
    <div id="editPwdWindow" class="easyui-window" title="修改成功" collapsible="false" minimizable="false" modal="true" closed="true" resizable="false"
        maximizable="false" icon="icon-save"  style="width: 300px; height: 160px; padding: 5px;
        background: #fafafa">
        <div class="easyui-layout" fit="true">

            <div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
				<div id="errorForNewPass" style="text-align: center;color: red">
				</div>
				<form id="editForm">
					<table cellpadding=3>
						<tr>
							<td>新密码：</td>
							<td><input id="txtNewPass" data-options="validType:'length[3,6]'" required="required" type="Password" class="txt01 easyui-validatebox" name="newPassword"/></td>
						</tr>
						<tr>
							<td>确认密码：</td>
							<td><input id="txtRePass" data-options="validType:'length[3,6]'" required="required" onkeyup="checkNewPassword()"  type="Password" class="txt01 easyui-validatebox" name="reNewPassword" /></td>
						</tr>
					</table>
				</form>
            </div>
            <div region="south" border="false" style="text-align: right; height: 30px; line-height: 30px;">
                <a id="btnEp" onclick="updatePassword()"  class="easyui-linkbutton"  href="javascript:void(0)" >确定</a>
